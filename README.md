...
> **App 源码**在另一个仓库：https://github.com/furrynaling-alt/linji-app （Kotlin，不含签名钥匙）
...
一台手机可以同时把状态发给**多个服务器**，每台各有自己的地址和口令：

1. 在**第二台服务器**上照样放一份 `bridge.php`（见第 2、4 节），给它**单独一个口令**。
2. App → 设置 → **App 桥** → 拉到底 → 点「**＋ 添加插件桥**」：填名字、地址（`https://<那台域名>/linji/bridge.php`）、口令。
3. 保存后立刻同步一次；每条插件桥都能**单独开关**，右边「上次」显示最近的 HTTP 结果（200 就是通的）。
4. 主桥和插件桥**互不影响**：主桥关着、插件桥开着也照样同步（状态发给所有开着的桥）。
5. 任意一条桥下发的指令（加待办 / 发通知 / 锁机 / 改设置）都会在这台手机上执行 —— 所以口令只给**你信得过**的服务器。

> 一台服务器也可以被多台手机接（各自配对各自的手机即可，`state.json` 里按设备记）。

## 📚 教学目录（5 篇，配合 README 看）

| 教学 | 内容 | 什么时候看 |
|---|---|---|
| [教学 1 · App 桥配对](教学-1-App桥配对.md) | 手机 ↔ 服务器自动同步；3 步配对；双向推待办/通知；排错 | **先看这个**（最稳的一条路）|
| [教学 2 · SSH 私钥配对](教学-2-SSH私钥配对.md) | 手机直接执行服务器命令；4 步配对；**国内连海外要走 443** 的完整做法 | 想在手机上跑命令时 |
| [教学 3 · 没有域名怎么办](教学-3-没有域名怎么办.md) | 只有 IP / 免费域名 + CF / ZeroTier 三种免费方案 | 没有域名时 |
| [教学 4 · 常见问题](教学-4-常见问题.md) | 配对、检查更新、隐私、国产系统保活、一键给 AI 的提示词 | 出问题时先查这个 |
| [教学 5 · 插件](教学-5-插件.md) | 设置里加「插件页」：跟自己的 AI 商量写 JSON，就有自己的首页小卡片（股票/服务器/任意接口）| 想加卡片时 |

> 不想自己看文档：App 里 **设置 → SSH 私钥连接服务器 → 「复制 AI 提示词」**，
> 整段粘给任意 AI（豆包/马维斯/ChatGPT），它会带着你一步步做完。
